...
def _my_superpixel_segments(img, vor_annos, dataset, compactness=30):
    '''
...
def _my_kmeans_segments(img, vor_annos):
    ...
    with warnings.catch_warnings():
        ...
        kmean = cluster.KMeans(3, init=np.array([center_nuclei, center_bg, center_unknown])).fit(img.reshape(-1, 3))
...
def mix_match(img, bound, center, ratio, rr2, cc2, alpha):
    ...
    kmean = cluster.KMeans(2, init=np.array([center_nuclei, center_bg])).fit(aug_img[rr2, cc2].reshape(-1, 3)) 
    segment = kmean.labels_.reshape(H, W) == 0
...
def _AbKmeans(img, center, ratio, bound, update_num, threshold, patience, alpha, verbose=False):
    ...
    while scale < bound :
        ...
        kmean = cluster.KMeans(2,init=np.array([center_nuclei, center_bg])).fit(region.reshape(-1, 3))
        segment = kmean.labels_.reshape(H, W) == 0
...
def _add_random_shift(ctd, r, border, rng):
    '''
    ...
    while True:
        t1 = x1 + rng.integers(-r, r+1)
        #print(t1)
        if t1 >= 0 and t1 < border and t1 != x1:
            break

    while True:
        t2 = x2 + rng.integers(-r, r+1)
        #print(t2)
...
def gen_pseudo_labels_MoNuSeg(img, path, mode_label, random_shift, rng):
    '''
    ...
    label_channel_2 = np.zeros((1000, 1000), dtype=np.uint8)
    ...
    for region in annos:
        ...
        if ctd[0] >= 0 and ctd[0] < 1000 and ctd[1] >= 0 and ctd[1] < 1000:
            if random_shift > 0:
                ctd = _add_random_shift(ctd, random_shift, 1000, rng)
...
def gen_pseudo_labels_TNBC(img, mask, mode_label, random_shift, rng):
    '''
    generate label data for training
    ...
    label_channel_2 = np.zeros((512, 512), dtype=np.uint8)
                                                               #channel 2, store selected pseudo labels
    ...
    for e in contours:
        ...
        if random_shift > 0:
            ctd = _add_random_shift(ctd, random_shift, 512, rng)
        label_channel_1[ctd[1], ctd[0]] = 1
...
def gen_pseudo_labels_CPM17(img, mask, mode_label, random_shift, rng):
    '''
    generate label data for training
    ...
    #draw pseudo ground truth  channel_2
    pseudo_gt_selector = {
        'Sp': lambda x, y: _my_superpixel_segments(x, y, 'CPM17'), 
        'Kmeans': lambda x, y: _my_kmeans_segments(x, y), 
        'AbKmeans_05': lambda x, y: _my_AbKmeans_segments(x, y, '05'), 
        'AbKmeans_1': lambda x, y: _my_AbKmeans_segments(x, y, '1'), 
        'AbKmeans_2': lambda x, y: _my_AbKmeans_segments(x, y, '2'), 
...
def to_npimg(torch_img):
    if torch_img.ndim == 4:
        torch_img = torch_img.squeeze(0)                #BCHW => CHW
    if torch_img.ndim == 3 and torch_img.shape[0] == 1:
        torch_img = torch_img.squeeze(0)                #CHW => HW
    
    if torch_img.ndim == 3 and torch_img.shape[0] == 3:
        torch_img = torch_img.permute(1, 2, 0)          #CHW => HWC

    return np.array(torch_img.to('cpu').detach())
